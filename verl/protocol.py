...
class DataProto:
    ...
    def repeat_deep(self, repeat_times=2, interleave=True):
        """
        Repeat the batch data a specified number of times.
        ...
        repeated_non_tensor_batch = {}
        for key, val in self.non_tensor_batch.items():
            # Check if this is an object array (contains Python objects like lists/dicts)
            if val.dtype == object:
                # For object arrays, manually create deep copies to avoid shared references
                if interleave:
                    # Create list with deep copies in interleaved order
                    repeated_list = []
                    for item in val:
                        for _ in range(repeat_times):
                            repeated_list.append(copy.deepcopy(item))
                    repeated_non_tensor_batch[key] = np.array(repeated_list, dtype=object)
                else:
                    # Create list with deep copies in tiled order
                    repeated_list = []
                    for _ in range(repeat_times):
                        for item in val:
                            repeated_list.append(copy.deepcopy(item))
                ...
                # For non-object arrays (numeric types), np.repeat/tile works fine
                if interleave:
                    repeated_non_tensor_batch[key] = np.repeat(val, repeat_times, axis=0)
                else:
                    repeated_non_tensor_batch[key] = np.tile(val, (repeat_times,) + (1,) * (val.ndim - 1))
